...
def mq():
    ...
    def on_connect(client, userdata, flags, rc):
        ...
        client.publish(topic,'{"machine":'+machine+',"speed":"'+ str(dbs[237][0]['current_value'])+'","actual_length":"'+str(dbs[237][4]['current_value'])+'"}')
...
def plc():
    global client,snapclient,dbs,mycursor
    cycle_id=0  
#Siemens datatypes
    ...
    while True: 
        while str(snapclient.get_cpu_state()) != "S7CpuStatusRun":
            try:
                ...
                subprocess.run(["service", "plc_to_mysql","restart"])
            continue   
...
thread1 = threading.Thread(target=plc)
thread1.start()

thread2 = threading.Thread(target=mq)
thread2.start()
